...
import '../runtime/sd_ffi_bindings.dart';
import '../memory/hive_service.dart';
import '../runtime/inference_service.dart';
import '../gateway/cloud_service.dart';
import '../runtime/local_image_service.dart';
  ...
  Future<String> _prepareVisionImagePath({
    ...
    if (resized == null) {
      ...
      final failedDecodeFile = File(
        '${tempDir.path}/astra_image_${DateTime.now().millisecondsSinceEpoch}_$originalName',
      );
    ...
    final tempDir = await getTemporaryDirectory();
    final file = File(
      '${tempDir.path}/astra_vision_${DateTime.now().millisecondsSinceEpoch}.jpg',
    );
